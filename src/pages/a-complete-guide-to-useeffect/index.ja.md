...
どういうことでしょう？ `count` は 状態の変化を何らかの方法で「監視」し、自動的にアップデートするのでしょうか？これは React を学ぶ時の最初の直感としては役に立ちますが、実は[正しいメンタルモデルではありません](https://overreacted.io/react-as-a-ui-runtime/).
...
**state をアップデートする度、React はコンポーネント関数を呼び出します。それぞれの render 結果は*関数内の定数である* `counter` の state の値を*見る*ことになります**
...
この動作がいまいちピンとこない方は、もっと現実的な例を想像してみてください：受取人 ID を state に保持するチャットアプリと、送るボタンなど。[この記事](https://overreacted.io/how-are-function-components-different-from-classes/)が理由を深掘りしていますが正解は 3 です。
...
この記事はエフェクトに関する記事のはずだったんですが、まだエフェクトのことについて話してませんでしたね！これから修正していきます。エフェクトには何の違いもないことがわかりました。
...
もしかしたら、 `count` の更新を `effect` 関数内で行うための、ある種の「データバインディング」や「監視」があるのではないでしょうか？ `count` は React がコンポーネント内に設定する変更可能な変数で、エフェクトが常に最新の値を見るようになっているのかもしれない？

いいえ、それは違います。

`count` は特定のコンポーネントの render 内では一定であることはすでにわかっています。イベントハンドラーは、カウントがスコープ内の変数であるため、自分が「属する」 render からカウントの状態を「見る」ことができます。エフェクトについても同様です。
...
## それぞれの render は**すべて**を保持している

**エフェクトはすべての render 後に実行され、概念的にはコンポーネント出力の一部であり、特定の render 内からの　state　と props を「見る」ということがわかりました。**
...
Hooks は JavaScript のクロージャに頼りきっているのに、class の実装がクロージャとよく関連づけられる[タイムアウト内に違う値が入る不思議な現象](https://wsvincent.com/javascript-closure-settimeout-for-loop/)による混乱に見舞われているのは皮肉なことだと思います。これは、この例における混乱の実際の原因が、クロージャそのものではなく、 mutation （Reactはクラスの `this.state` を最新の状態を指すように変異させる）にあるからです。
...
もちろん、エフェクトで定義されたコールバックの中で、キャプチャされた値ではなく、最新の値を *読みたい* 場合もあります。最も簡単な方法は、[この記事](https://overreacted.io/how-are-function-components-different-from-classes/)の最後のセクションで説明したように、 ref を使用することです。

過去のレンダリングにある関数から*未来の* props や state を読み取る場合、流れに逆らって泳ぐことになるので注意が必要です。間違ったことではありませんが（必要な場合もあります）、パラダイムから外れると「きれい」ではなくなってしまうかもしれません。これは意図的な結果であり、どのコードが壊れやすく、タイミングに依存しているかを強調するのに役立ちます。クラスでは、このようなことがあってもあまり目立ちません。
...
React で何かを mutate するという行為は風変わりに見えるかもしれません。しかし、これはまさに React 自身がクラス内で `this.state` を再割当てする方法です。特定の render でキャプチャーされた props と state とは違い、 `latestCount.current` は特定のコールバックで同じ値を参照できる保証はありません。定義上、いつでも mutate 可能なのです。これが、デフォルト mutate 可能ではなく、自分で mutate するかどうか選んで使う必要があります。
...
エフェクトを同じように考えるのが正解でしょう。**`useEffect` は、React tree の外にあるものを props と state に応じて _シンクロ_ してくれます。** 
...
これは、*mount/update/unmount* のメンタルモデルとは微妙に異なります。 これはしっかり理解しましょう。**コンポーネントが最初にレンダリングされるかどうかに応じて異なる動作をするエフェクトを書こうとしているなら、それは潮の流れに逆らっていることになります！** もし、結果が *過程* に頼ってしまっている場合は、シンクロに失敗しています。
...
**エフェクトでもこのようなことができないでしょうか？エフェクトを適用する必要がないときに、再実行しないようにできたらいいですね。**

例えば、 state が変化してコンポーネントが再レンダリングするような場合です：
...
なので、もしエフェクトの不必要な再実行を避けたい場合は、`useEffect`に依存配列（deps と呼ばれる）を第 2 引数に渡します：
...
**「関数の中身はわからないけれど、render scope 内で `name` しか使ってないことを約束するよ」と React に言ってるようなものです。**
...
// React は関数の中を覗くことはできませんが、depsを比較することはできます。
// この例では、 すべての deps が同じであるため、新しいエフェクトを実行する必要はありません。
```
...
「でも、 mount 時だけに実行したい！」とおっしゃるかもしれません。今のところ、覚えておいてほしいのは： deps を指定した場合、 **エフェクトで使用されるコンポーネント内のすべての値がそこにある必要があるということです。** props, state, 関数など、コンポーネント内のあらゆるものが含まれます。

ですが稀に問題を引き起こす場合もあります。例えば、無限 refetch ループに出くわしたり、ソケットが何度も作られたりすることがあります。 **このような問題を解決するには、依存関係を取り除くことではありません。** すぐに解決策を見ていきましょう。

ですが解決策に飛びつく前に、問題をよりよく理解しましょう。
...
例えば、毎秒 1 づつ increment していくカウンターコンポーネントを作成してるとしましょう。 class のメンタルモデルでは setInterval を一度だけセットして、一回破棄するのが直感的に思い浮かぶでしょう。こちらが実際に実装した[例](https://codesandbox.io/s/n5mjzjy9kl)です。このコードを `useEffect` のメンタルモデルに置き換えると、直感的に `[]` を deps に追加してしまいます。"一度だけ実行してほしい"ということですよね？
...
React に関わって数年、もう不必要な `props` を子に渡して親コンポーネントのカプセル化を壊す行為（そして数週間後になぜやる必要があったのかに気づく）に慣れてしまいました。
