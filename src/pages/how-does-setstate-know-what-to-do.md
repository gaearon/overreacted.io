...
Updating the DOM sounds like something React DOM would be responsible for, but we’re calling `this.setState()`, not something from React DOM. Our `React.Component` base class is defined inside React itself.
