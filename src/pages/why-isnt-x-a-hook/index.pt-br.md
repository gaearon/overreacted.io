...
Para relembrar você, aqui estão algumas coisas que os Hooks *são*:

* [`useState()`](https://reactjs.org/docs/hooks-reference.html#usestate) permite que você declare uma variável de estado.
* [`useEffect()`](https://reactjs.org/docs/hooks-reference.html#useeffect) permite que você declare um efeito secundário.
* [`useContext()`](https://reactjs.org/docs/hooks-reference.html#usecontext) permite que você leia informação de um contexto.
...
**Nota: Esse artigo não é uma imersão para aqueles que estão interessados em discussões sobre APIs. Você não precisa pensar em nada disso para ser produtivo com a React!**

---

Há duas importantes propriedades que queremos preservas nas APIs da React:

1. **Composição:** Os [Hooks customizados](https://reactjs.org/docs/hooks-custom.html) são a razão principal por estarmos entusiasmados com a API dos Hooks. Esperamos que as pessoas criem seus prórpios Hooks frequentemente, e precisamos ter certeza que os Hooks escritos por pessoas diferentes [não entrem em conflito](/why-do-hooks-rely-on-call-order/#flaw-4-the-diamond-problem). (Não estamos todos mimados em como os componentes conseguem se compor de forma limpa e não quebram um ao outro?)

2. **Depuração:** Quremos que os erros sejam [fáceis de se encontrar](/the-bug-o-notation/) a medida que a aplicação cresce. Uma das melhores funcionalidades da React é que se você ver algo renderizado da maneira incorreta, você pode percorrer a árvore acima até encontrar qual prop ou estado de componente que causou o erro.
...
Adicionar uma nova chamada incondicional a `useState()` é sempre seguro. Você não precisa saber nada sobre os outros Hooks usados por um componente para declarar uma nova variável de estado. Você também não pode quebrar outras variáveis de estado ao atualizar uma delas.

**Veredito:** ✅ `useState()` não deixam os Hooks customizados frágeis.

### Depuração

Os Hooks são úteis porque você pode passar valores *entre* eles:
...
Não importa se você o chama `useShouldComponentUpdate()`, `usePure()`, `useSkipRender()` ou `useBailout()`, a proposta deve se parecer com algo assim:
...
Digamos que o rótulo (*label*) `Typing...` não apareça quando se espera, apesar de que em algum lugar muitos níveis acima a prop está sendo alterada. Como depuramos isso?

**Normalmente, em React você pode responder essa questão com confiança olhando *para os níveis de cima*.** Se `ChatThread` não obtém um novo valor `isTyping`, podemos abrir o componente que renderiza `<ChatThread isTyping={myVar} />` e checar `myVar`, e assim por diante. Em algum desses níveis, ou vamos encontrar uma implementação errada de `shouldComponentUpdate()`, ou um valor incorreto de `isTyping` sendo passado para baixo. Apenas uma verificada em cada componente da cadeia é geralmente suficiente para localizar a origem do problema.

Contudo, se esse Hook `useBailout()` fosse real, você nunca saberia a razão pela qual uma atualização foi pulada até que você verificasse *cada um dos Hooks customizados* (em profundidade) usado pelo nosso componente `ChatThread` e os componentes em suas cadeias de renderização. Visto que todo componente pai pode *também* utilizar Hooks customizados, isso iria tomar uma [proporção terrível](/the-bug-o-notation/)
...
Enquanto não há uma "versão Hook" de `memo()` ou `shouldComponentUpdate()`, React *fornece* um Hook chamado [`useMemo()`](https://reactjs.org/docs/hooks-reference.html#usememo). Serve para um propósito similar, mas sua semântica é suficientemente diferente para não cair nas armadilhas descritas anteriormente.
