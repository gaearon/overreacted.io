...
If the hosting is down, it would temporarily not resolve, but if the user gets it back up somewhere online and points their DID there, it will start resolving again. The user can also delete the record, which would remove it from the user's "everything folder".
...
To learn Next.js, I've made a little app called [Sidetrail](https://sidetrail.app/) a little while ago ([it's open source](https://tangled.org/danabra.mov/sidetrail)) which lets you create step-by-step walkthroughs and "walk" those.
